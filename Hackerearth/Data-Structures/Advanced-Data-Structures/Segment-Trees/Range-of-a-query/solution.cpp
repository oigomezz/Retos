...
vector<dataset> t(1200020);

void print(dataset x)
{
  cout << x.pref << " " << x.suff << " " << x.left << " " << x.right << " " << x.ans << endl;
}

dataset make_data(long long val)
{
  dataset res;
  res.pref = 1;
  res.suff = 1;
...
dataset query(long long v, long long tl, long long tr, long long l, long long r)
{
  if (l > r)
    return make_data(0);
  if (l == tl && r == tr)
